...
import { createUser, genUsername, expectToContainAllKeys } from "./testing-utilities";
...
describe("user endpoint", () => {
    ...
    test("allows searching for user by name", async () => {
        ...
        expect(rsp.status).toStrictEqual(200);
        expect(rsp.data).toBeInstanceOf(Array);
        expect((rsp.data as any[]).find((u) => u.name === user.username)).toBeDefined();
    });
    ...
    test("shows revoked permissions", async () => {
        ...
        expect(moment(rsp.data.permissions[perm].revoked_until).isAfter(moment())).toBeTruthy();
    });
});
