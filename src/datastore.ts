...
import type { GetNewsParms } from "./model/GetNewsParms";
...
import { DEFAULT_PERMISSIONS, type Permission } from "./model/Permission";
...
`;
	...
	> {
		...
		try {
			const resultSet = await db.execute(
				"SELECT g.`id` AS `game_id`, g.`name`, COALESCE(r.`difficulty`, NULL) AS `difficulty`, COALESCE(r.`rating`, NULL) AS `rating` FROM `Favorite` f, `Game` g JOIN `Rating` r ON r.`game_id` = g.`id` AND r.`user_id` = ? WHERE g.`id` = f.`game_id` AND f.`user_id` = ? AND g.`removed` IS FALSE",
				[uid, uid],
			);
	...
	> {
		...
		try {
			const resultSet = await db.execute(
				"SELECT g.`id` AS `game_id`, g.`name`, COALESCE(r.`difficulty`, NULL) AS `difficulty`, COALESCE(r.`rating`, NULL) AS `rating` FROM `Clear` c, `Game` g JOIN `Rating` r ON r.`game_id` = g.`id` AND r.`user_id` = ? WHERE g.`id` = c.`game_id` AND c.`user_id` = ? AND g.`removed` IS FALSE",
				[uid, uid],
			);
