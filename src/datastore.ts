...
`,
    ...
    async updateUser(user: any): Promise<boolean> {
        ...
        try {
            ...
            params.push(user.id);
            const rows = await database.execute(` UPDATE User ${updateList.getSetClause()} WHERE id = ?`, params);
    ...
    async updateGame(game: Game, isAdmin: boolean): Promise<boolean> {
        ...
        try {
            let params = updateList.getParams();
            params.push(game.id);
            const rows = await database.execute(` UPDATE Game ${updateList.getSetClause()} WHERE id = ?`, params);
    ...
    async addGame(game: Game, adderId: number): Promise<Game> {
        ...
        try {
            ...
            const result = await database.execute(`INSERT INTO Game ${insertList.getClause()}`, insertList.getParams());
...
`;
    ...
    async updateReport(report: Report): Promise<boolean> {
        ...
        try {
            ...
            const rows = await database.execute(` UPDATE Report ${updateList.getSetClause()} WHERE id = ?`, params);
            return rows.affectedRows == 1;
        } finally {
            database.close();
        }
    ...
    async addNews(article: News, adderId: number): Promise<Game> {
        ...
        try {
            ...
            const result = await database.execute(`INSERT INTO News ${insertList.getClause()}`, insertList.getParams());
    ...
    async updateNews(article: News): Promise<boolean> {
        ...
        try {
            ...
            const rows = await database.execute(` UPDATE News ${updateList.getSetClause()} WHERE id = ?`, params);
            return rows.affectedRows == 1;
        } finally {
            database.close();
        }
...
`;
    ...
    async addReview(review: Review, gameId: number, userId: number): Promise<Review> {
        const database = new Database();
        try {
            await database.execute(`DELETE FROM Rating WHERE game_id=? AND user_id=?`, [gameId, userId]);
    ...
    async updateReview(review: any): Promise<boolean> {
        ...
        try {
            ...
            const rows = await database.execute(` UPDATE Rating ${updateList.getSetClause()} WHERE id = ?`, params);
            return rows.affectedRows == 1;
        } finally {
            database.close();
        }
    ...
    async addLikeToReview(reviewId: number, userId: number): Promise<boolean> {
        ...
        try {
            await database.execute(` INSERT IGNORE INTO LikeReview (rating_id, user_id) VALUES (?,?)`, [
                reviewId,
                userId,
            ]);
    ...
    async removeLikeFromReview(reviewId: number, userId: number): Promise<boolean> {
        ...
        try {
            await database.execute(` DELETE FROM LikeReview WHERE rating_id = ? AND user_id = ?`, [reviewId, userId]);
            uncache(`review-likes-${reviewId}`);
            return true;
        } finally {
            database.close();
    ...
    async addFollowToUser(targetUserId: number, userId: number): Promise<boolean> {
        ...
        try {
            await database.execute(` INSERT IGNORE INTO UserFollow (user_id, user_follow_id) VALUES (?,?)`, [
                targetUserId,
                userId,
            ]);
            return true;
    ...
    async removeFollowFromUser(targetUserId: number, userId: number): Promise<boolean> {
        ...
        try {
            await database.execute(` DELETE FROM UserFollow WHERE user_id = ? AND user_follow_id = ?`, [
                targetUserId,
                userId,
            ]);
            return true;
    ...
    async addList(list: List, userId: number): Promise<List> {
        ...
        try {
            ...
            const result = await database.execute(`INSERT INTO List ${insertList.getClause()}`, insertList.getParams());
...
`,
    ...
    async addGameToList(listId: number, gameId: number): Promise<boolean> {
        ...
        try {
            ...
            await database.execute(`INSERT INTO ListGame ${ins.getClause()}`, ins.getParams());
            return true;
        } finally {
            database.close();
        }
...
`;
    ...
    async gameExists(id: number): Promise<boolean> {
        return await cache(`game-exists-${id}`, async () => {
            ...
            try {
                const res = await db.query("SELECT 1 FROM Game g WHERE g.id = ? AND g.removed = 0", [id]);
...
`;
    ...
    async updateScreenshot(ss: Screenshot, isAdmin: boolean): Promise<boolean> {
        ...
        try {
            let params = updateList.getParams();
            params.push(ss.id);
            const rows = await database.execute(` UPDATE Screenshot ${updateList.getSetClause()} WHERE id = ?`, params);
            return rows.affectedRows == 1;
        } finally {
            database.close();
        }
...
`;
    ...
    async setTags(gameId: number, userId: number, tags: number[]) {
        const database = new Database();
        try {
            await database.execute(`DELETE FROM GameTag WHERE game_id=? AND user_id=?`, [gameId, userId]);
            ...
            for (let tagId of tags) {
                ...
                await database.execute(`INSERT INTO GameTag ${insertList.getClause()}`, insertList.getParams());
            }
    ...
    async tagsExist(tagIds: number[]) {
        ...
        try {
            ...
            const res = await database.execute(`SELECT COUNT(1) as cnt FROM Tag WHERE id in (${qs})`, tagIds);
    ...
    async createTag(name: string) {
        ...
        try {
            ...
            const result = await database.execute(`INSERT INTO Tag ${insertList.getClause()}`, insertList.getParams());
    ...
    async getGames(params: GetGamesParms, countOnly?: boolean) {
        ...
        whereList.add2("g.date_created >= ?", new Date(params.createdFrom ?? new Date(0)));
        whereList.add2("g.date_created <= ?", new Date(params.createdTo ?? Date.now()));
...
WHERE t.name IN (${params.tags.map((s) => "?").join(",")})
        ...
        if (params.ratingTo !== undefined) havingList.add2("AVG(r.rating) <= ?", params.ratingTo);
        if (params.difficultyFrom !== undefined) havingList.add2("AVG(r.difficulty) >= ?", params.difficultyFrom);
        if (params.difficultyTo !== undefined) havingList.add2("AVG(r.difficulty) <= ?", params.difficultyTo);
...
`;
        try {
            const rows = await database.query(query, [params.page * params.limit, params.limit]);
            return rows;
        } finally {
            database.close();
        }
