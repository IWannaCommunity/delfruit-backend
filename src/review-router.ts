...
import { Body, Controller, Delete, Get, Header, Patch, Path, Put, Query, Response, Route, Security, SuccessResponse, Tags } from 'tsoa';
...
export class ReviewController extends Controller {
    ...
    @Get()
    public async getReviews(@Query() page?: number = 0, @Query() limit?: number = 50): Promise<Review[]> {
        // TODO: order by & dir
    ...
    @Response<void>(400, "Not Found")
    @Patch("{id}")
    public async patchReview(@Header("Authorization") authorization: string, @Path() id: number, @Body() requestBody: Review): Promise<void> {
    ...
    @Put("{id}/likes/{userId}")
    public async putReviewLike(@Header("Authorization") authorization: string, @Path() id: number, @Path() userId: number): Promise<void> {
        // NOTE: auth guard should make the error condition unreachable
        const user = extractBearerJWT(authorization);
    ...
    public async deleteReviewLike(@Header("Authorization") authorization: string, @Path() id: number, @Path() userId: number): Promise<void> {
        // NOTE: auth guard should make the error condition unreachable
        const user = extractBearerJWT(authorization);
    ...
    @Response<void>(404, "Not Found")
    @Get("{id}/likes/{userId}")
    public async deleteReviewLike(@Header("Authorization") authorization: string, @Path() id: number, @Path() userId: number): Promise<boolean> {
        // NOTE: auth guard should make the error condition unreachable
        const user = extractBearerJWT(authorization);
